    ...
    while not done:
        ...
        if done:
            ...
            if info["flag_get"]:
                ...
                # Unwrapping the environment to get the SkipFrame wrapper
                ...
                for j, (frame, action) in enumerate(zip(frames_log, actions_log)):
                    # upscale frame
                    scaling_factor = 10
                    new_dims = (frame.shape[1] * scaling_factor,
                                frame.shape[0] * scaling_factor)
                    frame = Image.fromarray(frame).resize(
                        new_dims, Image.NEAREST)

                    frame.save(os.path.join(
                        "games" f"game_{i}", f"frame_{j}.png"))
                    controllers[action].save(os.path.join(
                        "games" f"game_{i}", f"controller_{j}.png"))
